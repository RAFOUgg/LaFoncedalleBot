...
class MenuView(discord.ui.View):
    def __init__(self, all_products: List[dict]):
        ...
        self.hash_products = [p for p in self.all_products if any(kw.lower() in p.get('name', '').lower() for kw in hash_keywords)]
        self.weed_products = [p for p in self.all_products if not any(kw.lower() in p.get('name', '').lower() for kw in hash_keywords)]
        
        # Correction de l'assignation des custom_id
    ...
    async def weed_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            Logger.error(f"ERREUR DANS L'INTERACTION weed_button : {e}")
            traceback.print_exc()
            if not interaction.response.is_done():
    ...
    async def hash_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            Logger.error(f"ERREUR DANS L'INTERACTION hash_button : {e}")
            traceback.print_exc()
            if not interaction.response.is_done():
                await interaction.response.send_message("❌ Oups, une erreur est survenue.", ephemeral=True)

    def update_buttons(self):
        # S'assurer qu'il y a des enfants avant de les désactiver
        if len(self.children) >= 2:
            self.children[0].disabled = self.current_index == 0
            self.children[1].disabled = self.current_index >= len(self.products) - 1
    ...
    def create_embed(self) -> discord.Embed:
        ...
        # Couleur de l'embed en fonction du stock
        embed_color = discord.Color.dark_red() if product.get('is_sold_out') else discord.Color.from_rgb(255, 204, 0)
        
        # On utilise .get() partout pour plus de sécurité. La clé est maintenant 'detailed_description'
        embed = discord.Embed(
            title=f"{product.get('name', 'Produit inconnu')}",
            url=product.get('product_url', CATALOG_URL),
            description=product.get('detailed_description', "Aucune description."),
        ...
        # --- Section Stats (TAUX CBD, etc.) ---
        stats = product.get('stats', {})
        if stats:
            stats_text = ""
            for label, value in stats.items():
                stats_text += f"**{label} :** {value}\n"
            # On vérifie que la string n'est pas vide avant de l'ajouter
            if stats_text:
                embed.add_field(name="Caractéristiques", value=stats_text.strip(), inline=False)
...
class MenuView(discord.ui.View):
    def __init__(self, all_products: List[dict]):
        ...
        # --- CORRECTION FINALE ---
        # On accède à chaque bouton par son index dans la liste `self.children`
        # self.children[0] est le premier bouton ("Nos Fleurs")
        # self.children[1] est le deuxième bouton ("Nos Résines")
        self.children[0].custom_id = "menu_view_fleurs_button"
        self.children[1].custom_id = "menu_view_resines_button"
    ...
    async def weed_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            if not interaction.response.is_done():
                await interaction.response.send_message("❌ Oups, une erreur est survenue lors de l'affichage des fleurs.", ephemeral=True)
    ...
    async def hash_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            traceback.print_exc() # Affiche l'erreur complète dans la console
            if not interaction.response.is_done():
                await interaction.response.send_message("❌ Oups, une erreur est survenue lors de l'affichage des résines.", ephemeral=True)

# --- Vues pour la commande /top_noteurs ---
class TopRatersPaginatorView(discord.ui.View):
    """Vue pour paginer le classement des meilleurs noteurs."""
    # MODIFICATION : Changement du nombre d'items par page pour un affichage en ligne
    def __init__(self, raters_data: list, guild: discord.Guild, items_per_page: int = 10):
        super().__init__(timeout=300)
        self.raters_data = raters_data
        self.guild = guild
        self.items_per_page = items_per_page
        self.current_page = 0
        self.total_pages = (len(self.raters_data) - 1) // self.items_per_page

        self.update_buttons()

    def update_buttons(self):
        """Active ou désactive les boutons en fonction de la page actuelle."""
        if len(self.children) >= 2:
            self.children[0].disabled = self.current_page == 0
            self.children[1].disabled = self.current_page >= self.total_pages

    def create_embed_for_page(self) -> discord.Embed:
        """Génère l'embed pour la page de noteurs actuelle."""
        start_index = self.current_page * self.items_per_page
        end_index = start_index + self.items_per_page
        page_items = self.raters_data[start_index:end_index]

        embed = create_styled_embed(
            title="🌟 Classement des Critiques de la Communauté",
            # On laisse la description vide, car on va la remplir avec notre liste
            description="",
        ...
        leaderboard_text = ""
        medals = ["🥇", "🥈", "🥉"]
        for i, (user_id, user_name, rating_count, global_avg, min_note, max_note) in enumerate(page_items):
            rank = start_index + i
            prefix = medals[rank] if rank < 3 else f"**`{rank + 1}.`**"
            
            member = self.guild.get_member(user_id)
            display_name = member.mention if member else user_name

            # --- DÉCOMPOSITION DE L'AFFICHAGE ---

            # Ligne 1 : Rang, Nom et Nombre de notations
            # On ajoute le pluriel à "notation" si nécessaire
            plural_s = 's' if rating_count > 1 else ''
            user_line = f"{prefix} {display_name} (**{rating_count}** notation{plural_s})"

            # Ligne 2 : Le reste des statistiques, avec l'effet de citation
            stats_line = (
                f"> 📊 Moyenne : **{global_avg:.2f}/10** - [📉 : `{min_note:.1f}` |  📈 : `{max_note:.1f}`]" # On utilise \n pour mettre min/max sur une sous-ligne
            )
            
            # Assemblage final pour cet utilisateur
            leaderboard_text += f"{user_line}\n{stats_line}\n\n" # On ajoute deux \n pour espacer avec l'utilisateur suivant
        
        # On met le texte final dans la description de l'embed
    ...
    async def update_message(self, interaction: discord.Interaction):
        """Met à jour le message avec la nouvelle page et les bons boutons."""
        self.update_buttons()
...
class ProductSelectForGraph(discord.ui.Select):
    def __init__(self, products: List[str]):
        options = [discord.SelectOption(label=p) for p in products[:25]]
        super().__init__(placeholder="Choisissez un produit pour le graphique...", min_values=1, max_values=1, options=options)

    async def callback(self, interaction: discord.Interaction):
        await self.view.handle_graph_product_selection(interaction, self.values[0])

class ProductSelectViewForGraph(discord.ui.View):
    def __init__(self, products: List[str], bot: commands.Bot):
        super().__init__(timeout=180)
        self.bot = bot
        self.add_item(ProductSelectForGraph(products))

    async def handle_graph_product_selection(self, interaction: discord.Interaction, product_name: str):
        Logger.info(f"[{interaction.user.name}] Début de handle_graph_product_selection pour '{product_name}'.")
        await interaction.response.defer()
        await log_user_action(interaction, f"a demandé le graphique pour le produit '{product_name}'")
        chart_path = await self.bot.loop.run_in_executor(executor, graph_generator.create_radar_chart, product_name)

        if chart_path and os.path.exists(chart_path):
            Logger.info(f"[{interaction.user.name}] Graphique généré et trouvé à : {chart_path}. Tentative d'envoi...")
            try:
                await interaction.edit_original_response(content=f"Voici le graphique des notes pour **{product_name}** :", view=None)

                # Correction : ouvrir le fichier en mode binaire pour discord.File
                discord_file = discord.File(chart_path, filename=os.path.basename(chart_path))
                await interaction.followup.send(file=discord_file)

                Logger.success(f"[{interaction.user.name}] Fichier graphique envoyé avec succès.")
                await asyncio.to_thread(os.remove, chart_path)
                Logger.info(f"[{interaction.user.name}] Fichier graphique '{chart_path}' supprimé après envoi.")

            except discord.Forbidden:
                Logger.error(f"[{interaction.user.name}] Erreur de permission: Le bot n'a pas les permissions d'envoyer des fichiers dans le salon. Salon ID: {interaction.channel.id}")
                await interaction.followup.send("❌ Je n'ai pas la permission d'envoyer des fichiers dans ce salon. Veuillez vérifier mes permissions.", ephemeral=True)
            except discord.HTTPException as http_e:
                Logger.error(f"[{interaction.user.name}] Erreur Discord HTTP lors de l'envoi du graphique : {http_e} (Code: {http_e.status}, Message: {http_e.text})")
                traceback.print_exc()
                await interaction.followup.send("❌ Une erreur est survenue avec Discord lors de l'envoi du graphique. (Code d'erreur HTTP)", ephemeral=True)
            except Exception as e:
                Logger.error(f"[{interaction.user.name}] Erreur inattendue lors de l'envoi du graphique ou de la suppression du fichier : {e}")
                traceback.print_exc()
                await interaction.followup.send("❌ Une erreur est survenue lors de l'envoi du graphique. (Erreur générale)", ephemeral=True)
        else:
            Logger.warning(f"[{interaction.user.name}] La génération du graphique a échoué ou le fichier n'a pas été trouvé pour '{product_name}'. Chemin: {chart_path}")
            await interaction.edit_original_response(content=f"Désolé, impossible de générer un graphique pour **{product_name}**. Il n'y a peut-être pas assez de notes pour ce produit ou une erreur est survenue.", view=None)


class RatingView(discord.ui.View):
    def __init__(self, product_name: str, user: discord.User, bot: commands.Bot):
        super().__init__(timeout=300)
        self.product_name = product_name; self.user = user; self.bot = bot; self.message: discord.InteractionMessage = None
        self.categories = ["Visuel", "Odeur", "Touché", "Goût", "Effets"]
        self.category_keys = ["visual_score", "smell_score", "touch_score", "taste_score", "effects_score"]
        self.current_step = 0; self.ratings = {}
    
    def create_embed(self) -> discord.Embed:
        if self.current_step >= len(self.categories):
            embed = discord.Embed(title="Merci pour votre contribution !", description=f"Vos notes pour **{self.product_name}** ont bien été enregistrées.", color=discord.Color.green())
        else:
            current_category = self.categories[self.current_step]
            embed = discord.Embed(title=f"Notation pour : {self.product_name}", description=f"Étape **{self.current_step + 1}/{len(self.categories)}** : Donnez une note pour le critère **{current_category}**.", color=discord.Color.blurple())
        if self.ratings:
            embed.add_field(name="Vos notes", value="\n".join([f"✅ **{name}** : {score}/10" for name, score in self.ratings.items()]), inline=False)
        return embed

    @discord.ui.button(label="Donner ma note", style=discord.ButtonStyle.primary)
    async def rate_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        modal = RatingModal(self, bot=self.bot); await interaction.response.send_modal(modal)

class RatingModal(discord.ui.Modal, title="Donnez votre note"):
    score_input = discord.ui.TextInput(label="Note (Ex: 7 ou 7.5)", placeholder="Point ou virgule acceptés.", max_length=4)

    def __init__(self, parent_view: RatingView, bot: commands.Bot):
        super().__init__()
        self.view = parent_view; self.bot = bot
        self.score_input.label = f"Note pour '{self.view.categories[self.view.current_step]}' (0-10)"

    def save_to_db_sync(self): # Renommée pour plus de clarté
        conn = sqlite3.connect(DB_FILE); cursor = conn.cursor()
        final_scores = {key: self.view.ratings.get(name) for key, name in zip(self.view.category_keys, self.view.categories)}
...
ON CONFLICT(user_id, product_name) DO UPDATE SET
    ...
    touch_score=excluded.touch_score,
    taste_score=excluded.taste_score,
    effects_score=excluded.effects_score,
    rating_timestamp=excluded.rating_timestamp
''', (
    self.view.user.id,
    self.view.user.name,
    self.view.product_name,
    final_scores.get("visual_score"),
    final_scores.get("smell_score"),
    final_scores.get("touch_score"),
    final_scores.get("taste_score"),
    final_scores.get("effects_score"),
    datetime.utcnow().isoformat()
))
        conn.commit(); conn.close()
        Logger.success(f"Données de {self.view.user.name} pour {self.view.product_name} sauvegardées en DB.")

    async def on_submit(self, interaction: discord.Interaction):
        try:
            # --- Valider l'entrée utilisateur ---
            try:
                score_value = float(self.score_input.value.replace(',', '.'))
                if not 0 <= score_value <= 10: raise ValueError("Score hors de l'intervalle 0-10")
            except (ValueError, TypeError):
                return await interaction.response.send_message("Veuillez entrer un nombre valide entre 0 et 10.", ephemeral=True)

            # --- Mettre à jour l'état et avancer à l'étape suivante ---
            self.view.ratings[self.view.categories[self.view.current_step]] = score_value
            self.view.current_step += 1

            # --- Vérifier si la notation est terminée ---
            if self.view.current_step >= len(self.view.categories):
                # OUI, C'EST LA FIN !
                
                # On lance les opérations en arrière-plan
                await asyncio.to_thread(self.save_to_db_sync) # Appel direct via asyncio.to_thread
                await log_user_action(interaction, f"a terminé de noter le produit '{self.view.product_name}'")
                
                # On crée notre embed final de remerciement
                final_embed = discord.Embed(
                    title="✅ Merci pour votre contribution !",
                    description=f"Vos notes pour **{self.view.product_name}** ont bien été enregistrées et nous aideront beaucoup à sélectionner les meilleurs produits pour vous.",
                    color=discord.Color.green()
                )
                
                # On ajoute un récapitulatif des notes pour confirmation
                scores_text = "\n".join([f"**{name}** : {score}/10" for name, score in self.view.ratings.items()])
                final_embed.add_field(name="🥇 Récapitulatif de vos notes", value=scores_text, inline=False)
                final_embed.set_footer(text="Vous pouvez maintenant rejeter ce message ou modifier votre avis avec /noter.")
                
                # On met à jour le message avec le nouvel embed ET on retire les boutons (view=None)
                await interaction.response.edit_message(embed=final_embed, view=None)
                return # On arrête la fonction ici, car c'est terminé.

            # --- Si ce n'est pas la fin, on met à jour l'embed pour l'étape suivante ---
            embed = self.view.create_embed()
            await interaction.response.edit_message(embed=embed, view=self.view)

        except Exception as e:
            Logger.error("="*50); Logger.error(f"ERREUR NON GÉRÉE DANS on_submit:"); traceback.print_exc(); Logger.error("="*50)
            if not interaction.response.is_done():
                await interaction.response.send_message("❌ Une erreur interne est survenue.", ephemeral=True)
            else:
                await interaction.followup.send("❌ Une erreur interne est survenue après la réponse initiale.", ephemeral=True)

class OverwriteRatingConfirmView(discord.ui.View):
    def __init__(self, product_name, user, bot):
        super().__init__(timeout=30)
        self.product_name = product_name
        self.user = user
        self.bot = bot

    @discord.ui.button(label="Écraser donnée", style=discord.ButtonStyle.danger)
    async def overwrite(self, interaction: discord.Interaction, button: discord.ui.Button):
        view = RatingView(self.product_name, self.user, self.bot)
        await interaction.response.edit_message(
            content=f"Notation pour **{self.product_name}** :",
            embed=view.create_embed(),
            view=view
        )
...
# --- Ajoutez une vue dédiée pour la sélection de produit à noter ---
class NotationProductSelect(discord.ui.Select):
    def __init__(self, products, user, bot):
        options = [discord.SelectOption(label=p) for p in products[:25]]
        self.user = user
        self.bot = bot
        super().__init__(placeholder="Choisissez un produit à noter...", min_values=1, max_values=1, options=options)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        def has_already_rated():
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT COUNT(*) FROM ratings WHERE user_id = ? AND LOWER(TRIM(product_name)) = LOWER(TRIM(?))",
        ...
        already_rated = await asyncio.to_thread(has_already_rated)

        if already_rated:
            warning = f"⚠️ Vous avez déjà noté **{product_name}**. Cette action remplacera votre note précédente.\n\n"
            await interaction.response.edit_message(
                content=warning + "Que souhaitez-vous faire ?",
                embed=None,
                view=OverwriteRatingConfirmView(product_name, self.user, self.bot)
            )
        else:
            view = RatingView(product_name, self.user, self.bot)
            await interaction.response.edit_message(
                content=f"Notation pour **{product_name}** :",
                embed=view.create_embed(),
                view=view
            )
...
class NotationProductSelectView(discord.ui.View):
    def __init__(self, products, user, bot):
        ...
        self.add_item(NotationProductSelect(products, user, bot))

class NotesView(discord.ui.View):
    def __init__(self, notes, user=None, can_reset=False, bot=None):
        super().__init__(timeout=120)
        self.notes = notes
        self.user = user
    ...
    def create_embed(self):
        embed = discord.Embed(
            title=f"Notes de {self.user.display_name if self.user else 'vous'}",
            description="Voici les notes attribuées aux produits :",
            color=discord.Color.blurple()
        )
        for note in self.notes:
            scores = (
                f"Visuel: {note['visual_score']}/10\n"
                f"Odeur: {note['smell_score']}/10\n"
                f"Touché: {note['touch_score']}/10\n"
                f"Goût: {note['taste_score']}/10\n"
                f"Effets: {note['effects_score']}/10"
            )
            embed.add_field(
                name=f"{note['product_name']} ({note['rating_timestamp'][:10]})",
                value=scores,
                inline=False
            )
        if not self.notes:
            embed.description = "Aucune note trouvée."
        return embed

    @discord.ui.button(label="Réinitialiser toutes les notes", style=discord.ButtonStyle.danger, custom_id="reset_notes_btn")
    async def reset_notes_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not self.can_reset or not self.user:
            await interaction.response.send_message("Vous n'avez pas la permission.", ephemeral=True)
            return
        # Confirmation
        await interaction.response.send_message(
            f"Êtes-vous sûr de vouloir supprimer toutes les notes de {self.user.mention} ? (Cette action est irréversible)",
            view=ConfirmResetNotesView(self.user, self.bot),
            ephemeral=True
        )

class ProfilePaginatorView(discord.ui.View):
    """Vue pour afficher le profil d'un utilisateur et paginer ses notes."""
    def __init__(self, target_user: discord.Member, user_stats: dict, user_ratings: list, can_reset: bool, bot: commands.Bot):
        super().__init__(timeout=300)
        ...
        self.current_page = 0
        self.items_per_page = 3 # Afficher 3 notes détaillées par page
        self.total_pages = (len(self.user_ratings) - 1) // self.items_per_page

        self.update_components()

    def update_components(self):
        """Met à jour les boutons en fonction de la page et des permissions."""
        # On supprime les anciens boutons avant de les recréer
        self.clear_items()
        
        # Pagination des notes
        if self.user_ratings and self.total_pages > 0:
            self.add_item(self.PrevButton(self))
            self.add_item(self.NextButton(self))
            # On met à jour l'état des boutons de pagination
            self.children[0].disabled = self.current_page == 0
            self.children[1].disabled = self.current_page >= self.total_pages
    ...
    def create_embed(self) -> discord.Embed:
        """Génère l'embed complet du profil."""
        embed = discord.Embed(
            title=f"Profil de {self.target_user.display_name}",
            color=self.target_user.accent_color or discord.Color.blurple()
        )
        ...
        stats_text = (
            f"**🏆 Rang Global :** `#{self.user_stats.get('rank', 'Non classé')}`\n"
            f"**📝 Notations :** `{self.user_stats.get('count', 0)}`\n"
            f"**📊 Moyenne :** `{self.user_stats.get('avg', 0):.2f}/10`"
        )
        if self.user_stats.get('is_top_3_monthly', False):
             stats_text += "\n**🏅 Badge :** Critique Expert du Mois"

        embed.add_field(name="Statistiques du Critique", value=stats_text, inline=False)
        embed.add_field(name="\u200b", value="**Dernières Notations :**", inline=False) # Titre pour la section des notes

        # --- Section 2: Pagination des notes détaillées ---
        if not self.user_ratings:
            embed.add_field(name="", value="Cet utilisateur n'a pas encore noté de produit.", inline=False)
        else:
            start_index = self.current_page * self.items_per_page
            end_index = start_index + self.items_per_page
            page_items = self.user_ratings[start_index:end_index]
            
            for note in page_items:
                avg_note = (note['visual_score'] + note['smell_score'] + note['touch_score'] + note['taste_score'] + note['effects_score']) / 5.0
                timestamp = datetime.fromisoformat(note['rating_timestamp']).strftime('%d/%m/%Y')
                embed.add_field(
                    name=f"**{note['product_name']}** - Note : {avg_note:.1f}/10",
                    value=f"*Noté le {timestamp}*",
                    inline=False
                )
...
class SlashCommands(commands.Cog):
    ...
    async def menu(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Construire l'embed du menu
            general_promos_text = "\n".join([f"• {promo}" for promo in site_data.get('general_promos', [])]) or "Aucune promotion générale en cours."
            hash_count, weed_count = get_product_counts(products) # Utilise la fonction de shared_utils
            embed = discord.Embed(
            ...
            # Envoyer le message en privé à l'utilisateur
            await interaction.followup.send(embed=embed, view=view, ephemeral=True)
